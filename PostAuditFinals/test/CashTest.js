...
// Contract constants
const Token               = artifacts.require('../contracts/CashToken.sol')
...
/*********************************** Mint ****************************************/
  ...
  describe('mint', function () {
    ...
    it('should mint 1000 tokens to accountTwo', async function () {

      await this.token.mint(this.accountTwo, toEther(1000), {from: this.owner})
      let twoBalance = await this.token.balanceOf(this.accountTwo)
      twoBalance.should.be.bignumber.equal(toEther(1000))

    })

    it('should add 1000 tokens to total supply', async function () {

      await this.token.mint(this.accountTwo, toEther(1000), {from: this.owner})
      let totalSupply = await this.token.totalSupply()
      totalSupply.should.be.bignumber.equal(toEther(107143857))

    })

    it('should emit Mint event', async function () {

      let tx = await this.token.mint(this.accountTwo, toEther(1), {from: this.owner})
      tx.logs[0].event.should.be.equal("Mint")

    })

    it('should fail on 0 amount', async function () {

      await this.token.mint(this.accountTwo, 0, {from: this.accountTwo}).should.be.rejectedWith(EVMRevert);

    })

    it('should fail on non-owner call', async function () {

      await this.token.mint(this.accountTwo, toEther(10000), {from: this.accountTwo}).should.be.rejectedWith(EVMRevert);
...
/*********************************** Burn ****************************************/
  ...
  describe('burn', function () {
    ...
    it('should burn 1000 tokens from owner', async function () {
      ...
      await this.token.burn(this.owner, toEther(1000), {from: this.owner})
      let balance = await this.token.balanceOf(this.owner)
      balance.should.be.bignumber.equal(toEther(107141857))

    })

    it('should subtract 1000 tokens from total supply', async function () {

      await this.token.burn(this.owner, toEther(1000), {from: this.owner})
      let totalSupply = await this.token.totalSupply()
      totalSupply.should.be.bignumber.equal(toEther(107141857))

    })

    it('should emit Burn event', async function () {

      let tx = await this.token.burn(this.owner, toEther(1), {from: this.owner})
    ...
    it('should fail on 0 amount', async function () {
      ...
      await this.token.burn(this.owner, 0, {from: this.accountTwo}).should.be.rejectedWith(EVMRevert);

    })

    it('should fail if user doesn\'t have tokens to burn', async function () {

      await this.token.burn(this.accountTwo, 1, {from: this.owner}).should.be.rejectedWith(EVMRevert);

    })

    it('should fail on non-owner call', async function () {

      await this.token.burn(this.owner, toEther(10000), {from: this.accountTwo}).should.be.rejectedWith(EVMRevert);
...
/****************************** receiveApproval ********************************/
  ...
  describe('receiveApproval', function () {
    ...
    it('approveAndCallPreSigned should succeed with token2 as payment and pay original sender', async function() {
      ...
      let preSignedHash = await this.token.getPreSignedHash('0xcae9ca51', this.token2.address, toEther(10), data, 0, 1)
      let signature = web3.eth.sign(this.owner, preSignedHash);

      let typesArray = ['bytes', 'address', 'uint256', 'bytes', 'uint256', 'uint256']
      let parameters = [signature, this.token2.address, toEther(10), data, 0, 1]
      let preRaw = web3Abi.encodeParameters(typesArray, parameters);
...
/******************************** Constants ************************************/
  ...
  describe('Constants', function () {
    ...
    it('should return the correct name', async function() {
      ...
      name.should.be.equal("Coinvest CASH Token")
